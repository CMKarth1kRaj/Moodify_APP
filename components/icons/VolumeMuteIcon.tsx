
import React from 'react';

export const VolumeMuteIcon: React.FC<React.SVGProps<SVGSVGElement>> = (props) => (
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" strokeWidth="0" strokeLinecap="round" strokeLinejoin="round" {...props}>
        <polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"></polygon>
        <line x1="23" y1="9" x2="17" y2="15" strokeWidth="2" stroke="currentColor" fill="none"></line>
        <line x1="17" y1="9" x2="23" y2="15" strokeWidth="2" stroke="currentColor" fill="none"></line>
    </svg>
);
